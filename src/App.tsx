...
var PERMS={admin:["dashboard","vehicles","rentals","customers","expenses","calendar"],manager:["dashboard","vehicles","rentals","customers","expenses","calendar"],sale:["dashboard","rentals","customers","calendar"],accountant:["dashboard","expenses"]};
var WRITES={vehicles:["admin","manager"],rentals:["admin","manager","sale"],customers:["admin","manager","sale"],expenses:["admin","manager","accountant"]};
var RL={admin:"Quản trị",manager:"Quản lý",sale:"Kinh doanh",accountant:"Kế toán"};
var PS=10;
var fm=function(n){return n?n.toLocaleString("vi-VN"):"0"};
var fd=function(s){try{return new Date(s).toLocaleDateString("vi-VN")}catch(e){return s}};
...
var iD=function(d){return new Date(Date.now()+d*864e5).toISOString().split("T")[0]};
...
/* ── UI Components ── */
function Toast(p){if(!p.t)return null;var err=p.t.type==="error";return(<div className={"fixed top-4 right-4 z-[60] px-5 py-3 rounded-xl shadow-xl text-white text-sm font-medium flex items-center gap-2 "+(err?"bg-red-500":"bg-green-500")}>{err?<XCircle className="w-4 h-4"/>:<CheckCircle className="w-4 h-4"/>}{p.t.msg}</div>)}
function Spinner(){return(<div className="fixed inset-0 bg-black/20 z-50 flex items-center justify-center"><div className="bg-white px-6 py-4 rounded-xl shadow-xl flex items-center gap-3"><div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"/>Đang xử lý...</div></div>)}
function Md(p){return(<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-3" onClick={p.onClose}><div className={"bg-white rounded-2xl w-full max-h-[92vh] overflow-y-auto p-5 sm:p-6 "+(p.wide?"max-w-4xl":"max-w-lg")} onClick={function(e){e.stopPropagation()}}><div className="flex justify-between items-center mb-4"><h2 className="text-lg font-bold truncate pr-4">{p.title}</h2><button onClick={p.onClose} className="text-gray-400 hover:text-gray-600 shrink-0"><X className="w-5 h-5"/></button></div>{p.children}</div></div>)}
function FI(p){var c="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 "+(p.error?"border-red-400":"border-gray-300");return(<div className={p.className||""}>{p.label&&<label className="block text-sm font-medium text-gray-700 mb-1">{p.label}</label>}{p.textarea?<textarea data-testid={p.testid} value={p.value||""} onChange={function(e){p.onChange(e.target.value)}} placeholder={p.placeholder} className={c} rows={2}/>:<input data-testid={p.testid} type={p.type||"text"} value={p.value||""} onChange={function(e){p.onChange(e.target.value)}} placeholder={p.placeholder} className={c}/>}{p.error&&<p className="text-red-500 text-xs mt-1">{p.error}</p>}</div>)}
function SI(p){return(<div className={p.className||""}>{p.label&&<label className="block text-sm font-medium text-gray-700 mb-1">{p.label}</label>}<select data-testid={p.testid} value={p.value||""} onChange={function(e){p.onChange(e.target.value)}} className={"w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 "+(p.error?"border-red-400":"border-gray-300")}><option value="">{p.placeholder||"-- Chọn --"}</option>{p.options.map(function(o){return(<option key={o.value} value={o.value}>{o.label}</option>)})}</select>{p.error&&<p className="text-red-500 text-xs mt-1">{p.error}</p>}</div>)}
function Rw(p){return(<div className="flex justify-between py-0.5"><span className="text-gray-500 text-sm">{p.l}</span><span className={"text-sm font-medium "+(p.rc||"")}>{p.r}</span></div>)}
function Bg(p){var c={green:"bg-green-100 text-green-700",orange:"bg-orange-100 text-orange-700",red:"bg-red-100 text-red-700",blue:"bg-blue-100 text-blue-700",gray:"bg-gray-100 text-gray-700"};return(<span className={"px-2 py-0.5 rounded-full text-xs font-semibold "+(c[p.color]||c.gray)}>{p.children}</span>)}
function SC(p){var c={blue:"bg-blue-100 text-blue-600",green:"bg-green-100 text-green-600",orange:"bg-orange-100 text-orange-600",purple:"bg-purple-100 text-purple-600"};return(<div className="bg-white rounded-xl shadow-sm border p-4"><div className="flex items-center justify-between"><div><p className="text-gray-500 text-xs sm:text-sm">{p.t}</p><p className="text-xl sm:text-2xl font-bold mt-1">{p.v}</p>{p.s&&<p className="text-xs text-gray-400 mt-0.5">{p.s}</p>}</div><div className={"p-2.5 rounded-xl "+(c[p.c]||"")}>{p.icon}</div></div></div>)}
function Pg(p){var pages=Math.ceil(p.total/PS);if(pages<=1)return null;return(<div className="flex items-center justify-center gap-2 mt-4"><button disabled={p.page<=1} onClick={function(){p.onChange(p.page-1)}} className="px-3 py-1.5 rounded-lg border text-sm disabled:opacity-40 hover:bg-gray-50">‹</button><span className="text-sm text-gray-600">{p.page}/{pages}</span><button disabled={p.page>=pages} onClick={function(){p.onChange(p.page+1)}} className="px-3 py-1.5 rounded-lg border text-sm disabled:opacity-40 hover:bg-gray-50">›</button></div>)}
function CM(p){return(<Md onClose={p.onCancel} title="Xác nhận"><p className="text-sm text-gray-600 mb-4">{p.msg}</p><div className="flex gap-2"><button onClick={p.onConfirm} className="flex-1 bg-red-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-red-700">Xác nhận</button><button onClick={p.onCancel} className="flex-1 bg-gray-200 py-2 rounded-lg text-sm">Hủy</button></div></Md>)}
...
export default function App(){
  ...
  var canWrite=useCallback(function(p){return user&&(WRITES[p]||[]).indexOf(user.role)>=0},[user]);
  var vMap=useMemo(function(){var m={};vehicles.forEach(function(v){m[v.id]=v});return m},[vehicles]);
  var cMap=useMemo(function(){var m={};customers.forEach(function(c){m[c.id]=c});return m},[customers]);
  var vN=useCallback(function(id){return(vMap[id]||{}).name||""},[vMap]);
  var vP=useCallback(function(id){return(vMap[id]||{}).plate||""},[vMap]);
  var cN=useCallback(function(id){return(cMap[id]||{}).name||""},[cMap]);
  var cPh=useCallback(function(id){return(cMap[id]||{}).phone||""},[cMap]);
  var chkOvl=useCallback(function(vid,sd,ed,ex){return rentals.some(function(r){return r.vehicle_id===vid&&r.status==="active"&&r.id!==ex&&r.start_date<=ed&&r.end_date>=sd})},[rentals]);
  var getVS=useCallback(function(vid,date){var r=rentals.find(function(r){return r.vehicle_id===vid&&r.status==="active"&&r.start_date<=date&&r.end_date>=date});if(!r)return{status:"available",time:null,rental:null};return{status:"rented",time:date===r.end_date?r.end_time:null,rental:r}},[rentals]);

  var handleLogin=function(){
    if(!lf.u||!lf.p){setLe("Vui lòng nhập đầy đủ");return}
    var pw={admin:"admin123",sale1:"sale123",manager:"manager123",accountant:"acc123"};
  ...
  if(!isAuth){return(
    <div className="min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md">
      ...
      <div className="space-y-3">
        <FI label="Tên đăng nhập" value={lf.u} onChange={function(v){setLf({u:v,p:lf.p})}} testid="login-user"/>
        <FI label="Mật khẩu" type="password" value={lf.p} onChange={function(v){setLf({u:lf.u,p:v})}} testid="login-pass"/>
  ...
  return(
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-30"><div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 flex justify-between items-center">
        ...
        <button onClick={function(){setIsAuth(false);setUser(null)}} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 text-sm shrink-0"><LogOut className="w-4 h-4"/><span className="hidden sm:inline">Thoát</span></button>
      </div></header>
        ...
        <div className="max-w-7xl mx-auto px-1 sm:px-6">
          <div className="flex overflow-x-auto py-1" style={{gap:"2px"}}>
            {tabs.map(function(t){
              var Icon=t.I;
              var a=tab===t.k;
              var cls="flex flex-col items-center px-2 py-1.5 rounded-lg whitespace-nowrap transition min-w-0 ";
              var clsDesktop="sm:flex-row sm:gap-2 sm:px-4 sm:py-3 ";
              var clsActive=a?"bg-blue-50 text-blue-600 font-semibold":"text-gray-500 hover:text-gray-700 hover:bg-gray-50";
              return(
                <button key={t.k} onClick={function(){setTab(t.k)}} className={cls+clsDesktop+clsActive}>
...
/* ══════════════════════════════════════════════════════ */
/* DASHBOARD — DOANH THU TỪNG XE + LỌC NGÀY            */
/* ══════════════════════════════════════════════════════ */
function DashTab(p){
  var[dateFrom,setDateFrom]=useState("");
  var[dateTo,setDateTo]=useState("");
  var[selVeh,setSelVeh]=useState(null);

  var filteredRentals=useMemo(function(){
    return p.rentals.filter(function(r){
  ...
  var filteredExpenses=useMemo(function(){
    return p.expenses.filter(function(e){
  ...
  var vehRevenue=useMemo(function(){
    var m={};
    p.vehicles.forEach(function(v){m[v.id]={revenue:0,expense:0,trips:0,surcharge:0}});
    filteredRentals.forEach(function(r){
      if(m[r.vehicle_id]){
        m[r.vehicle_id].revenue+=r.total;
        m[r.vehicle_id].surcharge+=(r.surcharge||0);
        m[r.vehicle_id].trips+=1;
      }
    });
  ...
  var expiring=p.rentals.filter(function(r){var t=td();var i3=iD(3);return r.status==="active"&&r.end_date>=t&&r.end_date<=i3}).sort(function(a,b){return a.end_date.localeCompare(b.end_date)});

  var detailRentals=selVeh?filteredRentals.filter(function(r){return r.vehicle_id===selVeh.id}):[];
  var detailExpenses=selVeh?filteredExpenses.filter(function(r){return r.vehicle_id===selVeh.id}):[];

  var filterLabel=(!dateFrom&&!dateTo)?"Toàn bộ thời gian":(dateFrom||"...")+" → "+(dateTo||"...");
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm text-gray-600 font-medium">Lọc theo ngày:</span>
          <input type="date" value={dateFrom} onChange={function(e){setDateFrom(e.target.value)}} className="px-3 py-1.5 border rounded-lg text-sm"/>
          <span className="text-gray-400">→</span>
          <input type="date" value={dateTo} onChange={function(e){setDateTo(e.target.value)}} className="px-3 py-1.5 border rounded-lg text-sm"/>
          {(dateFrom||dateTo)&&(
            <button onClick={function(){setDateFrom("");setDateTo("")}} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1"><X className="w-3 h-3"/>Xóa lọc</button>
          )}
            ...
            return(
              <div key={v.id} onClick={function(){setSelVeh(isSelected?null:v)}} className={"rounded-xl border p-4 cursor-pointer transition hover:shadow-md "+(isSelected?"border-blue-500 bg-blue-50 shadow-md":"border-gray-200 bg-white hover:border-gray-300")}>
                ...
                <div className="space-y-1">
                  <Rw l="Doanh thu" r={fm(rev)+"đ"} rc="text-green-600"/>
                  <Rw l="Chi phí" r={"-"+fm(data.expense)+"đ"} rc="text-red-500"/>
                  <div className="border-t pt-1 mt-1">
                    <Rw l="Lợi nhuận" r={fm(net)+"đ"} rc={net>=0?"text-green-700 font-bold":"text-red-600 font-bold"}/>
                  </div>
                  <Rw l="Số chuyến" r={data.trips}/>
                </div>
              </div>
            )
          })}
        ...
        <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-sm flex items-center gap-2">
              <span className="text-2xl">{selVeh.image}</span>
              Chi tiết: {selVeh.name} — {selVeh.plate}
            </h3>
            <button onClick={function(){setSelVeh(null)}} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5"/></button>
          </div>
          ...
          {detailRentals.length===0?(
            <p className="text-gray-400 text-sm py-4 text-center">Không có hợp đồng trong khoảng thời gian này</p>
          ):(
            <div className="overflow-x-auto mb-4">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500"><tr><th className="px-3 py-2 text-left">Khách</th><th className="px-3 py-2 text-left">Ngày thuê</th><th className="px-3 py-2 text-left">Ngày trả</th><th className="px-3 py-2 text-right">Tổng tiền</th><th className="px-3 py-2 text-right">Phụ thu</th><th className="px-3 py-2 text-left">Trạng thái</th></tr></thead>
                <tbody className="divide-y">
                  {detailRentals.map(function(r){
                    var stC=r.status==="active"?"orange":r.status==="completed"?"green":"red";
                    var stT=r.status==="active"?"Đang thuê":r.status==="completed"?"Xong":"Hủy";
                    return(
                      <tr key={r.id} className="hover:bg-gray-50">
                        <td className="px-3 py-2">{p.cN(r.customer_id)}</td>
                        <td className="px-3 py-2">{fd(r.start_date)}</td>
                        <td className="px-3 py-2">{fd(r.end_date)}</td>
                        <td className="px-3 py-2 text-right font-medium text-green-600">{fm(r.total)}đ</td>
                        <td className="px-3 py-2 text-right">{r.surcharge>0?<span className="text-orange-500">{fm(r.surcharge)}đ</span>:"—"}</td>
                        <td className="px-3 py-2"><Bg color={stC}>{stT}</Bg></td>
                      </tr>
                    )
                  })}
                </tbody>
          ...
          {detailExpenses.length===0?(
            <p className="text-gray-400 text-sm py-4 text-center">Không có chi phí trong khoảng thời gian này</p>
          ):(
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500"><tr><th className="px-3 py-2 text-left">Ngày</th><th className="px-3 py-2 text-left">Loại</th><th className="px-3 py-2 text-left">Mô tả</th><th className="px-3 py-2 text-right">Số tiền</th></tr></thead>
                <tbody className="divide-y">
                  {detailExpenses.map(function(e){
                    return(
                      <tr key={e.id} className="hover:bg-gray-50">
                        <td className="px-3 py-2">{fd(e.date)}</td>
                        <td className="px-3 py-2"><Bg>{EXP_L[e.type]||e.type}</Bg></td>
                        <td className="px-3 py-2 text-gray-500">{e.description}</td>
                        <td className="px-3 py-2 text-right text-red-500 font-medium">-{fm(e.amount)}đ</td>
                      </tr>
                    )
                  })}
                </tbody>
      ...
      {/* Expiring contracts */}
      <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-5">
        <h3 className="font-semibold mb-3 flex items-center gap-2 text-sm"><Clock className="w-4 h-4 text-orange-500"/>Hợp đồng sắp hết hạn (3 ngày)</h3>
        {expiring.length===0?(
          <p className="text-gray-400 text-center py-6 text-sm">Không có</p>
        ):(
          <div className="space-y-2">{expiring.map(function(r){
            var v=p.vMap[r.vehicle_id];
            var dd=Math.ceil((new Date(r.end_date)-new Date())/864e5);
            return(
              <div key={r.id} className="flex items-center justify-between p-3 bg-orange-50 rounded-xl border border-orange-100">
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-3xl shrink-0">{v?v.image:"🚗"}</span>
                  <div className="min-w-0">
                    <p className="font-semibold text-sm truncate">{p.cN(r.customer_id)}</p>
                    <p className="text-xs text-gray-500 truncate">{v?v.name:""} — {v?v.plate:""}</p>
                    <p className="text-xs text-gray-400">Trả: <span className="text-orange-600 font-medium">{fd(r.end_date)} {r.end_time}</span></p>
                  </div>
                </div>
                <Bg color={dd<=0?"red":"orange"}>{dd<=0?"Hôm nay":dd===1?"Ngày mai":dd+" ngày"}</Bg>
              </div>
            )
          })}</div>
        )}
...
function VehTab(p){
  ...
  var add=function(){
    var e={};if(!f.name.trim())e.name="Bắt buộc";if(!f.plate)e.plate="Bắt buộc";
    else if(!/^\d{2}[A-Z]-\d{4,5}$/.test(f.plate.toUpperCase()))e.plate="Sai (VD: 30A-12345)";
    else if(p.vehicles.some(function(v){return v.plate===f.plate.toUpperCase()}))e.plate="Đã tồn tại";
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <div className="relative"><Search className="w-4 h-4 absolute left-2.5 top-2.5 text-gray-400"/><input value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="Tìm xe..." className="pl-8 pr-3 py-2 border rounded-lg text-sm w-40"/></div>
          <select value={fSt} onChange={function(e){setFSt(e.target.value)}} className="px-3 py-2 border rounded-lg text-sm"><option value="all">Tất cả</option><option value="available">Sẵn sàng</option><option value="rented">Đang thuê</option></select>
      ...
      {showAdd&&(<div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <FI label="Tên xe *" value={f.name} onChange={function(v){setF(Object.assign({},f,{name:v}))}} error={errors.name}/>
          <FI label="Biển số *" value={f.plate} onChange={function(v){setF(Object.assign({},f,{plate:v.toUpperCase()}))}} error={errors.plate} placeholder="30A-12345"/>
          <FI label="Giá/ngày *" type="number" value={f.price_day} onChange={function(v){setF(Object.assign({},f,{price_day:v}))}} error={errors.price_day}/>
          <FI label="Giá/tuần" type="number" value={f.price_week} onChange={function(v){setF(Object.assign({},f,{price_week:v}))}} placeholder="Tự tính"/>
          <FI label="Giá/tháng" type="number" value={f.price_month} onChange={function(v){setF(Object.assign({},f,{price_month:v}))}} placeholder="Tự tính"/>
          <FI label="ODO (km)" type="number" value={f.odo} onChange={function(v){setF(Object.assign({},f,{odo:v}))}}/>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {filtered.map(function(v){
          ...
          if(isE){return(
            <div key={v.id} className="bg-white rounded-xl shadow-sm border p-4"><div className="space-y-2">
              <FI label="Tên xe" value={editing.name} onChange={function(val){setEditing(Object.assign({},editing,{name:val}))}}/>
              <FI label={"ODO ≥ "+fm(v.odo)} type="number" value={String(editing.odo)} onChange={function(val){setEditing(Object.assign({},editing,{odo:val}))}}/>
              <FI label="Giá/ngày" type="number" value={String(editing.price_day)} onChange={function(val){setEditing(Object.assign({},editing,{price_day:val}))}}/>
              <div className="flex gap-2 pt-1"><button onClick={saveEdit} className="flex-1 bg-green-600 text-white py-1.5 rounded-lg text-sm">Lưu</button><button onClick={function(){setEditing(null)}} className="flex-1 bg-gray-200 py-1.5 rounded-lg text-sm">Hủy</button></div>
            </div></div>
          )}
          ...
          return(
            <div key={v.id} className="bg-white rounded-xl shadow-sm border p-4 hover:shadow-md transition">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2 min-w-0"><span className="text-3xl">{v.image}</span><div className="min-w-0"><h3 className="font-semibold text-sm truncate">{v.name}</h3><p className="text-gray-500 text-xs">{v.plate}</p></div></div>
                <Bg color={sc}>{st}</Bg>
              </div>
...
/* ══ RENTALS ══ */
function RenTab(p){
  var[showAdd,setShowAdd]=useState(false);var[retM,setRetM]=useState(null);var[canM,setCanM]=useState(null);
  ...
  var co=p.customers.map(function(c){return{value:c.id,label:c.name+" — "+c.phone}});
  var vo=p.vehicles.filter(function(v){return v.status==="available"}).map(function(v){return{value:v.id,label:v.name+" — "+v.plate}});
  ...
  return(
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <div className="relative"><Search className="w-4 h-4 absolute left-2.5 top-2.5 text-gray-400"/><input value={search} onChange={function(e){setSearch(e.target.value);setPage(1)}} placeholder="Tìm..." className="pl-8 pr-3 py-2 border rounded-lg text-sm w-36"/></div>
          <select value={filter} onChange={function(e){setFilter(e.target.value);setPage(1)}} className="px-3 py-2 border rounded-lg text-sm"><option value="all">Tất cả</option><option value="active">Đang thuê</option><option value="completed">Xong</option><option value="cancelled">Hủy</option></select>
      ...
      {showAdd&&(<div className="bg-green-50 border border-green-200 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <FI label="Tiền cọc" type="number" value={f.deposit} onChange={function(v){setF(Object.assign({},f,{deposit:v}))}}/>
          <FI label="Địa điểm nhận" value={f.pickup_location} onChange={function(v){setF(Object.assign({},f,{pickup_location:v}))}}/>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden"><div className="overflow-x-auto">
        ...
        <tbody className="divide-y">{pg.map(function(r){
      ...
      {retM&&(<Md onClose={function(){setRetM(null)}} title="Trả xe"><div className="space-y-3">
        ...
        <FI label={"ODO trả ≥ "+fm(retM.odo_start)} type="number" value={rf.odo_end} onChange={function(v){setRf(Object.assign({},rf,{odo_end:v}))}}/>
        <FI label="Phụ thu (đ)" type="number" value={rf.surcharge} onChange={function(v){setRf(Object.assign({},rf,{surcharge:v}))}}/>
        {parseInt(rf.surcharge)>0&&<FI label="Lý do" value={rf.surcharge_note} onChange={function(v){setRf(Object.assign({},rf,{surcharge_note:v}))}}/>}
...
/* ══ CUSTOMERS ══ */
function CusTab(p){
  var[showAdd,setShowAdd]=useState(false);var[selC,setSelC]=useState(null);var[search,setSearch]=useState("");var[page,setPage]=useState(1);
  var ef={name:"",phone:"",address:"",id_card:"",license:""};var[f,setF]=useState(Object.assign({},ef));var[errors,setErrors]=useState({});
  var add=function(){
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center">
          <div className="relative"><Search className="w-4 h-4 absolute left-2.5 top-2.5 text-gray-400"/><input value={search} onChange={function(e){setSearch(e.target.value);setPage(1)}} placeholder="Tìm KH..." className="pl-8 pr-3 py-2 border rounded-lg text-sm w-44"/></div>
      ...
      {showAdd&&(<div className="bg-green-50 border border-green-200 rounded-xl p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FI label="Họ tên *" value={f.name} onChange={function(v){setF(Object.assign({},f,{name:v}))}} error={errors.name}/>
          <FI label="SĐT *" value={f.phone} onChange={function(v){setF(Object.assign({},f,{phone:v.replace(/\D/g,"")}))}} error={errors.phone} placeholder="0901234567"/>
          <FI label="Địa chỉ" value={f.address} onChange={function(v){setF(Object.assign({},f,{address:v}))}}/>
          <FI label="CMND/CCCD" value={f.id_card} onChange={function(v){setF(Object.assign({},f,{id_card:v.replace(/\D/g,"")}))}} error={errors.id_card}/>
          <FI label="Bằng lái" value={f.license} onChange={function(v){setF(Object.assign({},f,{license:v}))}}/>
        </div>
        <div className="flex gap-2 mt-3"><button onClick={add} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 flex items-center gap-1.5"><Save className="w-4 h-4"/>Lưu</button><button onClick={function(){setShowAdd(false);setErrors({})}} className="bg-gray-200 px-4 py-2 rounded-lg text-sm">Hủy</button></div>
      </div>)}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {pg.map(function(c){var cr=p.rentals.filter(function(r){return r.customer_id===c.id});var sp=cr.reduce(function(s,r){return s+r.total},0);return(
      ...
      {selC&&(<Md onClose={function(){setSelC(null)}} title={selC.name}>
        ...
        <div className="space-y-1.5">{p.rentals.filter(function(r){return r.customer_id===selC.id}).map(function(r){
          return(<div key={r.id} className="flex justify-between items-center bg-gray-50 p-2 rounded-lg text-sm"><span className="truncate">{p.vN(r.vehicle_id)}</span><span className="text-xs text-gray-400 ml-2">{fd(r.start_date)}→{fd(r.end_date)}</span><Bg color={r.status==="active"?"orange":"green"}>{r.status==="active"?"Thuê":"Xong"}</Bg></div>)
        })}</div>
      </Md>)}
    </div>
  )
...
/* ══ EXPENSES ══ */
function ExpTab(p){
  var[showAdd,setShowAdd]=useState(false);var[delC,setDelC]=useState(null);var[fT,setFT]=useState("all");var[page,setPage]=useState(1);
  var ef={vehicle_id:"",type:"",amount:"",date:"",description:""};var[f,setF]=useState(Object.assign({},ef));var[errors,setErrors]=useState({});
  var add=function(){
  ...
  var fl=p.expenses.filter(function(e){return fT==="all"||e.type===fT}).sort(function(a,b){return b.date.localeCompare(a.date)});
  var pg=fl.slice((page-1)*PS,page*PS);
  return(
    <div className="space-y-5">
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <select value={fT} onChange={function(e){setFT(e.target.value);setPage(1)}} className="px-3 py-2 border rounded-lg text-sm">
            <option value="all">Tất cả</option><option value="fuel">Nhiên liệu</option><option value="maintenance">Bảo dưỡng</option><option value="repair">Sửa chữa</option><option value="insurance">Bảo hiểm</option><option value="wash">Rửa xe</option><option value="road_fee">Phí đường bộ</option><option value="fine">Phạt nguội</option><option value="other">Khác</option>
          </select>
      ...
      {showAdd&&(<div className="bg-red-50 border border-red-200 rounded-xl p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <SI label="Xe *" value={f.vehicle_id} onChange={function(v){setF(Object.assign({},f,{vehicle_id:v}))}} options={vo} error={errors.vehicle_id}/>
          <SI label="Loại *" value={f.type} onChange={function(v){setF(Object.assign({},f,{type:v}))}} options={EXP_O} error={errors.type}/>
          <FI label="Số tiền *" type="number" value={f.amount} onChange={function(v){setF(Object.assign({},f,{amount:v}))}} error={errors.amount}/>
          <FI label="Ngày *" type="date" value={f.date} onChange={function(v){setF(Object.assign({},f,{date:v}))}} error={errors.date}/>
          <FI label="Mô tả" value={f.description} onChange={function(v){setF(Object.assign({},f,{description:v}))}} textarea className="sm:col-span-2"/>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden"><div className="overflow-x-auto">
        ...
        <tbody className="divide-y">{pg.map(function(e){
          var v=p.vMap[e.vehicle_id];var tl=EXP_L[e.type]||e.type;
          return(<tr key={e.id} className="hover:bg-gray-50">
            <td className="px-3 py-2.5">{fd(e.date)}</td><td className="px-3 py-2.5">{v?v.name:""}</td><td className="px-3 py-2.5"><Bg>{tl}</Bg></td><td className="px-3 py-2.5 text-gray-500 truncate max-w-[200px]">{e.description}</td><td className="px-3 py-2.5 text-right text-red-500 font-medium">-{fm(e.amount)}đ</td>
            <td className="px-3 py-2.5">{p.cw&&(<button onClick={function(){setDelC(e)}} className="text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4"/></button>)}</td>
          </tr>)
        })}</tbody></table>
      </div>{pg.length===0&&<p className="text-center py-8 text-gray-400 text-sm">Không có</p>}</div>
      <Pg page={page} total={fl.length} onChange={setPage}/>
